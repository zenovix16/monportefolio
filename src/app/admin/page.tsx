...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-10">
        <p className="text-xs tracking-[0.3em] uppercase text-black/45 mb-2">Vue d&apos;ensemble</p>
        <h1 className="text-3xl font-bold text-[#14161A]">Dashboard</h1>
      </div>
        ...
        {stats
          ? cards.map((c) => (
              ...
              >
                ...
                <p className="text-3xl font-bold text-[#14161A] mb-1">{c.value}</p>
      ...
      <div className="glass rounded-2xl p-6">
        <p className="text-xs tracking-[0.3em] uppercase text-black/45 mb-4">Accès rapide</p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[
            ...
            <a
              key={link.label}
              href={link.href}
              className="text-sm text-black/60 hover:text-[var(--accent-light)] border border-black/[0.08] hover:border-[var(--accent)]/40 rounded-xl px-4 py-3 transition-all"
            >
              {link.label} →
            </a>
          ))}
